...
<?php
	...
	$IdModifTable = $_POST['IdModifTable'];
	
	$IdTable = str_replace(' ', '_', $IdTable); 
	$IdModifTable = str_replace(' ', '_', $IdModifTable); 
 ...
 // RENAME TABLE `hermes`.`nouvelle_table` TO `hermes`.`nouvelle_table_test`;
	...
	try{
        ...
        $sql = "RENAME TABLE `hermes`.`$IdTable` TO `hermes`.`$IdModifTable`;";        
				$dbco->exec($sql);
...
<?php
	header("Location: /Hermes/Hermes.php?IdPage=5&IdTable=$IdModifTable"); // Retour page original
	exit();
?>
